...
using Resto.Front.Api.TestPlugin.Config;
...
namespace Resto.Front.Api.TestPlugin
{
    ...
    internal sealed class ButtonsTester : IDisposable
    {
        ...
        public ButtonsTester()
        {
            ...
            Operations.AddButtonToPluginsMenu("TestPlugin: Show extended keyboard view", x => ShowExtendedKeyboardPopup(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Show input dialog", x => ShowInputDialog(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Show decimal input dialog", x => ShowDecimalInputDialog(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Show ok popup", x => ShowOkPopup(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Show error popup", x => ShowErrorPopup(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Show date numpad popup", x => ShowDateNumpadPopup(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Show calendar popup", x => ShowCalendarPopup(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Show date and time popup", x => ShowDateTimePopup(x.vm)),
            Operations.AddButtonToPluginsMenu("TestPlugin: Auth by user", x => AuthenticateByUser(x.vm)),

            Operations.AddButtonToClosedOrderScreen("TestPlugin: Show ok popup", x => ShowOkPopupOnClosedOrderScreen(x.vm), TriangleIcon),
            Operations.AddButtonToProductsReturnScreen("TestPlugin: Show ok popup", x => ShowOkPopupOnProductsReturnScreen(x.vm)),
            Operations.AddButtonToOrderEditScreen("TestPlugin Button", x =>
            {
                var guest = x.order.Guests.First();
                if (!x.vm.ShowYesNoPopup("Test Plugin", $"Do you want to rename the first guest “{guest.Name}”?"))
                    return;
                ...
                x.os.RenameOrderGuest(x.order.Guests.First().Id, newName, x.order, x.os.GetDefaultCredentials());
            }, IikoIcon),
            Operations.AddButtonToPaymentScreen("Test Plugin", false, true, x =>
            {
                x.vm.ShowOkPopup("Test plugin", "Payment screen click on sample button");
                var isChecked = !x.state.isChecked;
                var caption = isChecked ? "Sample Checked" : "Test Plugin";
            ...
            try
            {
                authUserWithoutPinLicenseSubscription = Licensing.AcquireSlot(21057201, ConfigService.Config.ClientId);
            }
            catch (Exception)
            {
                // ignored
        ...
        private static void ShowInputDialog(IViewManager viewManager)
        {
            ...
            var settings = new ExtendedInputDialogSettings
            {
                ...
                NumericInputMode = NumericInputMode.String,
        ...
        {
            Operations.AddNotificationMessage(message, "TestPlugin", timeout ?? TimeSpan.FromSeconds(3));
        }
        ...
        private void AuthenticateByUser(IViewManager viewManager)
        {
            try
            {
                if (authUserWithoutPinLicenseSubscription == null)
                {
                    authUserWithoutPinLicenseSubscription = Licensing.AcquireSlot(21057201, ConfigService.Config.ClientId);
                }
            }
            ...
            {
                viewManager.ShowClosePopup("Warning", e.Message);
                return;
            }

            const string userName = "admin";
            var userToAuthenticate = Operations.GetUsers().FirstOrDefault(u => u.Name == userName);
            if (userToAuthenticate is null)
            {
                viewManager.ShowClosePopup("Warning", $"User with name \"{userName}\" not found.");
                return;
            }
            ...
            try
            {
                credentials = Operations.AuthenticateByUser(userToAuthenticate, ConfigService.Config.ClientId);
            }
            catch (Exception e)
            {
                viewManager.ShowClosePopup("Warning", $"Authentication by user \"{userName}\" failed.{Environment.NewLine}Reason: {e.Message}");
                return;
            }
            IUser userFromCredentials;
            try
            {
                userFromCredentials = Operations.GetUser(credentials);
            }
            catch (Exception e)
            {
                viewManager.ShowClosePopup("Warning", $"Getting user \"{userName}\" by his credentials failed.{Environment.NewLine}Reason: {e.Message}");
                return;
            }

            if (!userToAuthenticate.Equals(userFromCredentials))
                viewManager.ShowErrorPopup($"User to authenticate \"{userToAuthenticate.Name}\" is not equal to user from credentials \"{userFromCredentials.Name}\".");
            else
                viewManager.ShowOkPopup("Success", $"Authentication by user \"{userName}\" completed successfully.");
        }

        private void ShowOkPopupOnClosedOrderScreen(IViewManager viewManager)
        {
            viewManager.ShowOkPopup("Popup Title", "Message shown from Test plugin.");
        }
