...
namespace Resto.Front.Api.TestPlugin.NotificationHandlers
{
        ...
        private static void OnBeforeDoCheque([NotNull] IOrder order, [NotNull] IReadOnlyCollection<IPaymentItem> paymentItems, [NotNull] IOperationService os, [CanBeNull] IViewManager vm)
        {
            ...
            if (vm != null)
            {
                ...
                if (string.IsNullOrWhiteSpace(externalData))
                {
                    ...
                    if (add)
                        os.AddOrderExternalData(externalDataKey, "some-external-data", false, order, os.GetDefaultCredentials());
                }
                else
                {
                    ...
                    if (delete)
                        os.DeleteOrderExternalData(externalDataKey, order, os.GetDefaultCredentials());
                }
            }
