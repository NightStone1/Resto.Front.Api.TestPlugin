...
namespace Resto.Front.Api.TestPlugin
{
    internal sealed class EditorTester : IDisposable
    {
        ...
        private const string PluginName = "Resto.Front.Api.TestPlugin";
        ...
        private void AddButton(string text, Action action)
        {
            ...
            button.Click += (s, e) =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    MessageBox.Show(message, "TestPlugin", MessageBoxButton.OK, MessageBoxImage.Error);
                }
            };
        ...
        private void PrintXReport()
        {
            ...
            try
            {
                ...
                if (PluginContext.Operations.Print(printer, doc))
                    MessageBox.Show("Report successfully printed", "TestPlugin", MessageBoxButton.OK, MessageBoxImage.None);
                else
                    MessageBox.Show("Print error", "TestPlugin", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            catch (Exception e)
            {
                MessageBox.Show(e.Message, "TestPlugin", MessageBoxButton.OK, MessageBoxImage.Error);
                throw;
            }
        }
        ...
        private void CreateAndSendDelivery()
        {
            ...
            {
                var answer = MessageBox.Show(window, "Payment time is set to BeforeSending, but delivery payment is not supported in API yet. Try it anyway?", "TestPlugin", MessageBoxButton.YesNo);
        ...
        {
            os.AddOrderExternalData(PluginName, "Test plugin external data", false, order, os.GetDefaultCredentials());

            var value = os.TryGetOrderExternalDataByKey(order, PluginName);
            MessageBox.Show($"Test plugin external data value: {value}");
        }
        ...
        private void PrintCheck()
        {
            ...
            var clickHandler = new RoutedEventHandler((sender, args) =>
            {
                ...
                if (PluginContext.Operations.Print(selectedPrinter, doc))
                    MessageBox.Show("Document successfully printed", "TestPlugin", MessageBoxButton.OK, MessageBoxImage.None);
                else
                    MessageBox.Show("Print error", "TestPlugin", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        private void CreateJournalEvents()
        {
            ...
            var journalEventHigh = editSession.CreateJournalEvent("TestPlugin", Severity.High, "Sample of high severity journal event", DateTime.Now);
            ...
            var journalEventMiddle = editSession.CreateJournalEvent("TestPlugin", Severity.Middle, "Sample of  middle severity journal event", DateTime.Now);
            ...
            var journalEventLow = editSession.CreateJournalEvent("TestPlugin", Severity.Low, "Sample of low severity journal event", DateTime.Now);
        ...
        private static void MarkOrderAsTab([NotNull] IOrder order, [NotNull] IOperationService os)
        {
            ...
            {
                MessageBox.Show("Marking order as tab supported only in tab mode.", "TestPlugin", MessageBoxButton.OK, MessageBoxImage.Error);
                return;
            }
        ...
        private void AddPublicExternalData()
        {
            ...
            PluginContext.Operations.AddOrderExternalData(PluginName, "Test plugin public external data", true, order, PluginContext.Operations.GetDefaultCredentials());
        }
        ...
        private void CreateKitchenOrder()
        {
            ...
            var commentDto = new KitchenOrderItemCommentDto("Test Plugin", false);
