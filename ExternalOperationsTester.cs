    ...
    internal static class ExternalOperationsTester
    {
        ...
        private sealed class CalculatorServer : IDisposable
        {
            ...
            public CalculatorServer()
            {
                ...
                {
                    PluginContext.Operations.RegisterExternalOperation<Tuple<int, int>, int>(nameof(CalculatorServer), nameof(Add), x => Add(x.request.Item1, x.request.Item2)),
                    PluginContext.Operations.RegisterExternalOperation<Tuple<int, int>, int>(nameof(CalculatorServer), nameof(Subtract), x => Subtract(x.request.Item1, x.request.Item2)),
                    PluginContext.Operations.RegisterExternalOperation<DivisionRequest, DivisionResponse>(nameof(CalculatorServer), nameof(Divide), x=>Divide(x.request), CustomBinder.Instance)
                };
            }
        ...
        private sealed class CalculatorClient
        {
            ...
            {
                var sum = PluginContext.Operations.CallExternalOperation<Tuple<int, int>, int>(21005108, "CalculatorServer", "Add", Tuple.Create(a, b));
                PluginContext.Log.Info($"{a} + {b} = {sum}");
            }

            public void TestSubtraction(int minuend, int subtrahend)
            {
                var diff = PluginContext.Operations.CallExternalOperation<Tuple<int, int>, int>(21005108, "CalculatorServer", "Subtract", Tuple.Create(minuend, subtrahend));
            ...
            public void TestDivision(int dividend, int divisor)
            {
                ...
                var response = PluginContext.Operations.CallExternalOperation<DivisionRequest, DivisionResponse>(21005108, "CalculatorServer", "Divide", request, binder: CustomBinder.Instance);
